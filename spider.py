...
LOGGER = logone.get_logger(__name__)
LOGGER.set_level(level=logone.INFO)
...
def sync():
    try:
        LOGGER.info('Running: ' + str(datetime.now()))
            ...
            for i in distinct_song_ids:
                ...
                try:
                    ...
                    if song_data:
                        ...
                        LOGGER.info('--Download failed: ' + PAGE_URL + '/' + new_playlist_dict[i]['url'])
        ...
        LOGGER.info('Done: %d songs' % len(songs_db))
    except Exception as e:
        LOGGER.exception(e)


def main():
    LOGGER.info('Scheduling: ' + str(datetime.now()))
    sched.start()
